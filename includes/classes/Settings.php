...
class Settings {
	...
	public static function input (array $args = [], bool $return = false) {
		...
		echo sprintf(
			'<input type="%s" name="%s" id="%s" class="%s" placeholder="%s" value="%s" %s/>', $args['type'],
			$args['name'], $args['id'], implode(' ', $args['class']), $args['placeholder'], $args['value'],
			$args['attributes']
		);
		if ( !empty($args['desc']) ) {
	...
	public static function textarea (array $args = []) {
		...
		echo sprintf(
			'<textarea name="%s" id="%s" class="%s" placeholder="%s" rows="%s">%s</textarea>', $args['name'],
			$args['id'], implode(' ', $args['class']), $args['placeholder'], $args['row'], $args['value']
		);
		if ( !empty($args['desc']) ) {
			echo sprintf('<p class="description">%s</p>', $args['desc']);
		}
	...
	public static function checkbox (array $args = []) {
		...
		echo sprintf(
			...
			$args['name'], $args['id'], implode(' ', $args['class']), $args['label']
		);
		if ( !empty($args['desc']) ) {
			echo sprintf('<p class="description">%s</p>', $args['desc']);
		}
